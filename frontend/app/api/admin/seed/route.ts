...
export async function POST(req: Request) {
  try {
    const { aetherAmount, xlmAmount, callerPubKey } = await req.json();

    if (!ADMIN_WALLETS.includes(callerPubKey)) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 403 });
    }
    if (!aetherAmount || !xlmAmount || parseFloat(aetherAmount) <= 0 || parseFloat(xlmAmount) <= 0) {
    ...
    const tokenStroops = String(Math.floor(parseFloat(aetherAmount) * 1e7));
